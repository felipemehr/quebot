...
class QueBotAuth {
    ...
    try {
      ...
      this.auth.onAuthStateChanged(async (user) => {
        if (user) {
          ...
          // Auto-close auth modal when user logs in (e.g. after Google popup)
          if (!user.isAnonymous) {
            this.hideAuthModal();
          }
        } else {
  ...
  updateUI() {
    ...
    // On mobile, keep sidebar closed after UI update (e.g. after login)
    if (window.innerWidth <= 768 && typeof UI !== 'undefined' && UI.elements && UI.elements.sidebar) {
      UI.closeSidebar();
    }
  }
    ...
    if (level === USER_LEVEL.FULL) {
      content = `
        <div class="auth-modal-content">
          <h3>\ud83d\udc4b Hola, ${this.escapeHtml(this.userProfile.name)}!</h3>
          <p>Conectado con Google</p>
          <p class="auth-email">${this.escapeHtml(this.userProfile.email)}</p>
      ...
      content = `
        <div class="auth-modal-content">
          <h3>\ud83d\udc4b Hola, ${this.escapeHtml(this.userProfile.name || 'amigo')}!</h3>
          <p>Tienes un registro ligero</p>
          <div class="auth-buttons">
            <button class="auth-btn primary" onclick="queBotAuth.signInWithGoogle();">Conectar con Google</button>
            <button class="auth-btn secondary" onclick="queBotAuth.hideAuthModal();">Cerrar</button>
          </div>
        </div>
      `;
      ...
      content = `
        <div class="auth-modal-content">
          ...
          <div class="auth-buttons">
            <button class="auth-btn google" onclick="queBotAuth.signInWithGoogle();">
